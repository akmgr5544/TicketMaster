...
description: >
  ...
  per-service projects under Tests/, hand-written fakes (Events, Users), ArchUnit
  rules, and the Testcontainers-backed Bookings integration tests. Use when
  designing a test strategy, writing or fixing tests, or improving coverage of
  critical paths.
memory: project
---
...
Testcontainers fixture, the isolation model, and the known gaps in the current suite.
...
| Test doubles | hand-written fakes for Events and Users only — Bookings has none (see below) | Moq, NSubstitute |
...
**Every Bookings handler test runs against real infrastructure.**
`Tests/Bookings/BookingIntegration` runs every command/query handler — ReserveTicket, MakeBooking,
EventSync, Payments and CustomerBookings — plus the EF/DI mechanics tests, against real Postgres and
Redis in Testcontainers. This replaced both a SQLite in-memory suite (which could prove EF and DI
behavior but not Redis locking, TTLs, or genuine transaction rollback) and the fake-based
`BookingApplication` project, which is gone along with its `Fakes/` folder — there is no hand-written
fake for a Bookings handler dependency anywhere in the solution. Load the `testing` skill before
working on `BookingIntegration` — it is the authority on the fixture, the isolation model, and the
known gaps in the current suite.
...
```
Tests/Bookings/  BookingApi  BookingArchitecture  BookingDomain  BookingIntegration
...
Bookings.Api         → BookingApi              Events.Application → EventsApplication
Events.Api           → EventsApi               Users.Api → ArchitectureTests
```
...
```bash
dotnet test Tests/Bookings/BookingIntegration/BookingIntegration.csproj --filter "FullyQualifiedName~EventSync"
...
3. **Pick the right project** — a domain invariant belongs in `*Domain`, exception-to-status mapping
   in `*Api`, a structural rule in `*Architecture`. For Bookings handlers specifically: every group —
   ReserveTicket, MakeBooking, EventSync, Payments and CustomerBookings — is on the fixture in
   `BookingIntegration`, alongside its EF/DI mechanics tests. For Events, a handler belongs in
   `EventsApplication` with fakes.
...
- Hand-written fakes — the ongoing approach for Events and Users; Bookings has none
...
- Add a hand-written fake for a Bookings handler dependency — the fixture replaced that pattern; see
  the `testing` skill's "Why the fakes went away"
